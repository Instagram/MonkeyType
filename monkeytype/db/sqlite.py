...
from monkeytype.tracing import CallTrace as CallTraceType
...
class SQLiteStore(CallTraceStore):
    ...
    def get_or_add_signature(self,
                             arg_type: str,
                             return_type: str,
                             yield_type: str) -> int:

        signature = self.get_signature(arg_type, return_type, yield_type)
        ...
        values = [arg_type, return_type, yield_type]
    ...
    def get_or_add_function(self, trace: CallTraceType) -> int:
        ...
        signature = self.get_or_add_signature(trace.arg_types,
                                              trace.return_type,
                                              trace.yield_type)
    ...
    def add_trace(self, trace: CallTraceType) -> None:
        func = self.get_or_add_function(trace)

        arg_type = trace.arg_types
        return_type = trace.return_type
        yield_type = trace.yield_type

        signature = self.get_or_add_signature(arg_type, return_type, yield_type)
    ...
    def filter(
        ...
        query = f"""
        SELECT func.qualname, module.name, sig.arg_type, sig.return_type, sig.yield_type
        ...
        for row in cur.fetchall():
            (qualname, module, arg_type, return_type, yield_type) = row
            trace_row = CallTraceRow(module,
                                     qualname,
                                     arg_type,
                                     return_type,
                                     yield_type)
