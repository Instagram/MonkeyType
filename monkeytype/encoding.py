...
def get_function_annotation(d: TypeDict,
                            typ: type,
                            store: CallTraceStore) -> type:

    traces = store.filter(d['module'], d['qualname'])
    traces = [trace.to_trace(store) for trace in traces]
...
def type_from_dict(d: TypeDict, store: CallTraceStore) -> type:
...
def type_from_json(typ_json: str, store: CallTraceStore) -> type:
...
def arg_types_from_json(arg_types_json: str,
                        store: CallTraceStore) -> Dict[str, type]:
    """Reify the encoded argument types from the format produced by arg_types_to_json."""
    arg_types = json.loads(arg_types_json)
    return {name: type_from_dict(type_dict, store) for name, type_dict in arg_types.items()}


TypeEncoder = Callable[[type], str]


def maybe_encode_type(encode: TypeEncoder,
                      typ: Optional[type]) -> Optional[str]:
    if typ is None:
        return None
    return encode(typ)
...
def maybe_decode_type(decode: TypeDecoder,
                      encoded: Optional[str],
                      store: CallTraceStore) -> Optional[type]:
...
class CallTraceRow(CallTraceThunk):
    ...
    @classmethod
    def from_trace(cls: Type[CallTraceRowT],
                   trace: CallTrace,
                   store: CallTraceStore) -> CallTraceRowT:
...
def serialize_traces(traces: Iterable[CallTrace],
                     store: CallTraceStore) -> Iterable[CallTraceRow]:
